...
(function() {
  ...
  // Valen season definitions
  function getValenSeasonEvents(year) {
    const isLeap = isValenLeap(year);
    const interTwoExtra = isLeap ? 2 : 0;
    
    // Season boundaries (day of year, 1-indexed)
    return [
      { 
        name: "Cold", 
        startDay: 163 + 59, // Second Len'tar day 60 = start 163 + 59 = 222
        endDay: isLeap ? 327 + 105 : 325 + 105, // Third Len'tar day 106 = start 325/327 + 105
        color: "#b8d4e8", 
        hover: `❄️ Cold Season (${isLeap ? 209 : 209} days)`
      },
      { 
        name: "Mini Hot", 
        startDay: isLeap ? 327 + 106 : 325 + 106, // Third Len'tar day 107
        endDay: isLeap ? 327 + 149 : 325 + 149, // Third Len'tar day 150
        color: "#f5a878", 
        hover: `🔥 Mini Hot Season (44 days)`
      },
      { 
        name: "Second Cold", 
        startDay: isLeap ? 477 : 475, // Inter Three day 1
        endDay: 1 + 42, // First Len'tar day 43
        color: "#a8c0d8", 
        hover: `❄️ Second Cold Season (55 days)`
      },
      { 
        name: "Warm", 
        startDay: 1 + 43, // First Len'tar day 44
        endDay: 1 + 130, // First Len'tar day 131
        color: "#d8c878", 
        hover: `🌤️ Warm Season (88 days)`
      },
      { 
        name: "Rainy", 
        startDay: 1 + 131, // First Len'tar day 132
        endDay: 163 + 12, // Second Len'tar day 13 (start 163 + 12 = 175)
        color: "#78b8d8", 
        hover: `🌧️ Rainy Season (44 days)`
      },
      { 
        name: "Second Warm", 
        startDay: 163 + 13, // Second Len'tar day 14
        endDay: 163 + 58, // Second Len'tar day 59
        color: "#c8d878", 
  ...
  function getValenSeasonForDate(year, dayOfYear) {
    ...
    for (const season of events) {
      if (season.startDay <= season.endDay) {
        ...
        // Wraps around year end (Second Cold wraps from Inter Three to First Len'tar)
        if (dayOfYear >= season.startDay || dayOfYear <= season.endDay) {
          return season;
        }
      }
  ...
  function getValenMonthInfo(year, dayOfYear) {
    ...
    for (let i = 0; i < months.length; i++) {
      ...
      if (dayOfYear >= m.start && dayOfYear < m.start + m.days) {
        return { index: i, name: m.name, days: m.days, start: m.start, type: m.type, dayInMonth: dayOfYear - m.start + 1 };
      }
    }
    return null;
  }
  ...
  const el = {
    ...
    earthMonthDisp: document.getElementById('earthMonthDisplay'),
    earthGrid: document.getElementById('earthCalendarGrid'),
    valenYear: document.getElementById('valenYear'),
    valenDay: document.getElementById('valenDay'),
    valenMonthDisp: document.getElementById('valenMonthDisplay'),
    valenDayRange: document.getElementById('valenDayRange'),
    valenContainer: document.getElementById('valenCalendarContainer'),
